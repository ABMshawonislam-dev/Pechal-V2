  ...
  return (
    <div className="groupholder">
      ...
      {glist.map(item=>(
          ...
          <div>
            <button onClick={()=>handleGroupJoin(item)} className="boxbtn">Join</button>
          </div>
        </div>
        ))}
      ...
      >
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            <TextField onChange={(e)=>setGname(e.target.value)} style={{marginBottom:"10px"}} id="outlined-basic" label="Group Name" variant="outlined" />
            
            <TextField onChange={(e)=>setGtag(e.target.value)} style={{marginBottom:"10px"}} id="outlined-basic" label="Group Tag" variant="outlined" /> <br/>
            <Button onClick={handleCreateGroup} variant="contained">Contained</Button>
          </Typography>
        </Box>
      </Modal>
    </div>
